...
func TestLPosList(t *testing.T) {
	...
	fmt.Println()
	if !bytes.Equal(res.ToBytes(), resp.NewArrayData([]resp.RedisData{resp.NewIntData(0), resp.NewIntData(3)}).ToBytes()) {
		t.Error("count lpos error")
	}
	res = lPosList(m, [][]byte{[]byte("lpos"), []byte("l1"), []byte("c"), []byte("count"), []byte("1")})
	if !bytes.Equal(res.ToBytes(), resp.NewArrayData([]resp.RedisData{resp.NewIntData(2)}).ToBytes()) {
		t.Error("count lpos error")
	}
	res = lPosList(m, [][]byte{[]byte("lpos"), []byte("l1"), []byte("b"), []byte("count"), []byte("1"), []byte("rank"), []byte("-1")})
	if !bytes.Equal(res.ToBytes(), resp.NewArrayData([]resp.RedisData{resp.NewIntData(4)}).ToBytes()) {
		t.Error("count lpos error")
	}

	//    test maxlen option
	res = lPosList(m, [][]byte{[]byte("lpos"), []byte("l1"), []byte("a"), []byte("maxlen"), []byte("2"), []byte("count"), []byte("0")})
	if !bytes.Equal(res.ToBytes(), resp.NewArrayData([]resp.RedisData{resp.NewIntData(0)}).ToBytes()) {
		t.Error("maxlen lpos error")
	}
	res = lPosList(m, [][]byte{[]byte("lpos"), []byte("l1"), []byte("d"), []byte("maxlen"), []byte("3")})
	if !bytes.Equal(res.ToBytes(), resp.NewBulkData(nil).ToBytes()) {
		t.Error("maxlen lpos error")
	}
}
...
func TestLRemList(t *testing.T) {
	...
	if !bytes.Equal(res.ToBytes(), resp.NewIntData(2).ToBytes()) {
		t.Error("lrem error")
	}
